...
export const IKLimb = forwardRef<IKLimbRef, IKLimbProps>(({
    ...
    useFrame(() => {
        if (!groupRef.current || !upperRef.current || !lowerRef.current || !endRef.current) return;
...
export const LookAt = forwardRef<LookAtRef, LookAtProps>(({
    target,
    config,
    children
}, ref) => {
...
export const ProceduralWalk = forwardRef<ProceduralWalkRef, ProceduralWalkProps>(({
    ...
    useFrame((_, delta) => {
        ...
        velocityRef.current.copy(bodyPos).sub(lastPositionRef.current).divideScalar(Math.max(delta, 0.001));
...
export const TailPhysics = forwardRef<TailPhysicsRef, TailPhysicsProps>(({
    ...
    useImperativeHandle(ref, () => ({
        ...
        reset: () => {
            const positions = segmentsRef.current.map((_, i) => 
                new THREE.Vector3(0, -segmentLength * (i + 1), 0)
            );
        ...
        return Array.from({ length: segmentCount }, (_, i) => (
            ...
            >
                {visualize && (
                    <>
                        <mesh>
                            <sphereGeometry args={[visualRadius * (1 - i / segmentCount * 0.5), 8, 8]} />
                            <meshStandardMaterial color={visualColor} />
                        </mesh>
                        {i < segmentCount - 1 && (
                            <mesh position={[0, -segmentLength / 2, 0]}>
                                <cylinderGeometry args={[visualRadius * 0.5, visualRadius * 0.5, segmentLength, 6]} />
                                <meshStandardMaterial color={visualColor} />
                            </mesh>
                        )}
                    </>
...
export const BreathingAnimation = forwardRef<BreathingAnimationRef, BreathingAnimationProps>(({
    ...
    useImperativeHandle(ref, () => ({
        ...
        setAmplitude: (a: number) => { amplitudeRef.current = a; }
    }));
...
export const BlinkController = forwardRef<BlinkControllerRef, BlinkControllerProps>(({
    ...
    useImperativeHandle(ref, () => ({
        blink: triggerBlink,
        setBlinking: (enabled: boolean) => { blinkingRef.current = enabled; }
    }));
