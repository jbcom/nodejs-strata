...
export const Joystick3D = forwardRef<Joystick3DRef, Joystick3DProps>(({
    ...
    useFrame((state, delta) => {
        ...
        if (isPressed.current) {
            ...
            const plane = new THREE.Plane(new THREE.Vector3(0, 1, 0), -position[1] - 0.3 * size);
        ...
        currentAxis.current.x += (targetAxis.current.x - currentAxis.current.x) * delta * lerpSpeed;
        currentAxis.current.y += (targetAxis.current.y - currentAxis.current.y) * delta * lerpSpeed;
...
export const GroundSwitch = forwardRef<GroundSwitchRef, GroundSwitchProps>(({
    ...
    const handlePointerDown = useCallback((e: ThreeEvent<PointerEvent>) => {
        ...
        onPress?.({
            type: 'press',
            axis: axis === 'x' ? { x: currentValue.current, y: 0 } : { x: 0, y: currentValue.current },
            force: 1,
            worldPosition: worldPos.current.clone(),
            timestamp: Date.now(),
        });
    ...
    const handlePointerUp = useCallback(() => {
        ...
        const snappedValue = currentValue.current > 0.3 ? 1 : currentValue.current < -0.3 ? -1 : 0;
    ...
    useFrame((state, delta) => {
        ...
        if (Math.abs(currentValue.current - prevValue) > 0.01) {
            groupRef.current.getWorldPosition(worldPos.current);
            onAxisChange?.(
                axis === 'x' ? { x: currentValue.current, y: 0 } : { x: 0, y: currentValue.current },
...
export const PressurePlate = forwardRef<PressurePlateRef, PressurePlateProps>(({
    ...
    useFrame((state, delta) => {
        ...
        currentDepth.current += (targetDepth.current - currentDepth.current) * delta * springiness;
        ...
        onAxisChange?.({ x: 0, y: currentDepth.current / activationDepth }, worldPos.current.clone());
    });
...
export const WallButton = forwardRef<WallButtonRef, WallButtonProps>(({
    ...
    useFrame((state, delta) => {
        ...
        buttonDepth.current += (targetDepth.current - buttonDepth.current) * delta * springSpeed;
        ...
        groupRef.current.getWorldPosition(worldPos.current);
        onAxisChange?.({ x: 0, y: buttonDepth.current / (size * 0.15) }, worldPos.current.clone());
    });
    ...
    return (
        <group ref={groupRef} position={position} rotation={rotation}>
            ...
            <mesh geometry={rimGeometry} position={[0, size * 0.2, 0]} rotation={[Math.PI / 2, 0, 0]}>
