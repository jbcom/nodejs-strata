...
export const CharacterController = forwardRef<CharacterControllerRef, CharacterControllerProps>(({
    ...
    const performJump = useCallback(() => {
        ...
        const impulse = calculateJumpImpulse(config.jumpForce, 9.81 * config.gravityScale, config.mass);
    ...
    useFrame((state, delta) => {
        ...
        if (jumpBufferRef.current > 0 && coyoteTimeRef.current > 0 && jumpCountRef.current < config.maxJumps) {
            performJump();
        }
        ...
        const accel = effectivelyGrounded ? config.acceleration : config.acceleration * config.airControl;
        const decel = effectivelyGrounded ? config.deceleration : config.deceleration * config.airControl;
        
        let targetVelX = worldMoveDir.x * maxSpeed;
        let targetVelZ = worldMoveDir.z * maxSpeed;
...
export const VehicleBody = forwardRef<VehicleBodyRef, VehicleBodyProps>(({
    ...
    useFrame((_, delta) => {
        ...
        steeringAngleRef.current += (targetSteer - steeringAngleRef.current) * Math.min(1, 5 * delta);
        ...
        if (input.accelerate && !input.handbrake) {
            ...
            rigidBodyRef.current.applyImpulse({ x: force.x * delta, y: 0, z: force.z * delta }, true);
        }
        ...
        if (input.brake) {
            ...
            rigidBodyRef.current.applyImpulse({ x: brakeForce.x * delta, y: 0, z: brakeForce.z * delta }, true);
        }
        
        if (input.handbrake) {
            rigidBodyRef.current.setLinvel({ x: vel.x * 0.98, y: vel.y, z: vel.z * 0.98 }, true);
        }
    ...
    return (
        ...
        >
            <CuboidCollider
                args={[config.chassisSize[0] / 2, config.chassisSize[1] / 2, config.chassisSize[2] / 2]}
...
export const Buoyancy = forwardRef<BuoyancyRef, BuoyancyProps>(({
    ...
    const defaultSamplePoints = useMemo<[number, number, number][]>(() => samplePoints || [
        [-0.5, 0, -0.5],
        [0.5, 0, -0.5],
        [-0.5, 0, 0.5],
        [0.5, 0, 0.5],
