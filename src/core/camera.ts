...
export function lerpVector3(a: THREE.Vector3, b: THREE.Vector3, t: number, out?: THREE.Vector3): THREE.Vector3 {
...
export function slerp(qa: THREE.Quaternion, qb: THREE.Quaternion, t: number, out?: THREE.Quaternion): THREE.Quaternion {
...
export function evaluateCatmullRom(
    ...
    result.x = s * (2 * p1.x + (-p0.x + p2.x) * localT +
        (2 * p0.x - 5 * p1.x + 4 * p2.x - p3.x) * t2 +
        (-p0.x + 3 * p1.x - 3 * p2.x + p3.x) * t3);
    
    result.y = s * (2 * p1.y + (-p0.y + p2.y) * localT +
        (2 * p0.y - 5 * p1.y + 4 * p2.y - p3.y) * t2 +
        (-p0.y + 3 * p1.y - 3 * p2.y + p3.y) * t3);
    
    result.z = s * (2 * p1.z + (-p0.z + p2.z) * localT +
